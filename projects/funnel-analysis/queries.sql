-- Funnel analysis for an e-commerce platform
-- Author: Manoj Kumar Kavuri | PostgreSQL
--
-- Table assumptions:
...
SELECT 'page_view' AS funnel_stage, page_views AS users, 100.0 AS stage_rate, 100.0 AS overall_cr
FROM funnel_counts
UNION ALL
...
ROUND(100.0 * checkout / NULLIF(page_views, 0), 2)
FROM funnel_counts
UNION ALL
SELECT 'purchase', purchase,
ROUND(100.0 * purchase / NULLIF(checkout, 0), 2),
ROUND(100.0 * purchase / NULLIF(page_views, 0), 2)
FROM funnel_counts;

-- 2. Drop-off by stage, absolute and percent
WITH stage_users AS (
  SELECT event_type AS stage,
  COUNT(DISTINCT user_id) AS unique_users,
  ROW_NUMBER() OVER (ORDER BY CASE event_type
...
SELECT curr.stage, curr.unique_users,
...
ROUND(100.0 * (LAG(curr.unique_users) OVER (ORDER BY curr.stage_order) - curr.unique_users)
  / NULLIF(LAG(curr.unique_users) OVER (ORDER BY curr.stage_order), 0), 2) AS drop_off_pct
...
ROUND(100.0 * COUNT(DISTINCT CASE WHEN event_type = 'purchase' THEN user_id END)
  / NULLIF(COUNT(DISTINCT CASE WHEN event_type = 'page_view' THEN user_id END), 0), 2) AS end_to_end_cr_pct
FROM events
...
-- 5. Weekly conversion trend
SELECT DATE_TRUNC('week', created_at)::DATE AS week_start,
COUNT(DISTINCT CASE WHEN event_type = 'page_view' THEN user_id END) AS page_views,
COUNT(DISTINCT CASE WHEN event_type = 'purchase' THEN user_id END) AS purchases,
ROUND(100.0 * COUNT(DISTINCT CASE WHEN event_type = 'purchase' THEN user_id END)
  / NULLIF(COUNT(DISTINCT CASE WHEN event_type = 'page_view' THEN user_id END), 0), 2) AS weekly_cr_pct
FROM events
GROUP BY DATE_TRUNC('week', created_at)
ORDER BY week_start;

-- 6. A/B test: control vs treatment checkout page
WITH ab_funnel AS (
  SELECT ab.variant,
...
SELECT variant, checkout_users, purchase_users,
ROUND(100.0 * purchase_users / NULLIF(checkout_users, 0), 2) AS checkout_to_purchase_cr_pct,
ROUND(100.0 * purchase_users / NULLIF(checkout_users, 0)
  - LAG(100.0 * purchase_users / NULLIF(checkout_users, 0)) OVER (ORDER BY variant), 2) AS uplift_vs_previous_pct
FROM ab_funnel
ORDER BY variant;

-- 7. Re-engagement candidates: reached checkout, never purchased
SELECT e.user_id,
...
WHERE e.user_id NOT IN (SELECT DISTINCT user_id FROM events WHERE event_type = 'purchase')
